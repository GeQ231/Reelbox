        ...
        <div class="like-section mt-2">
            @auth
                <div class="like-toggle" style="cursor:pointer;" data-postid="{{ $post->id }}">
                    <i class="bi me-1 {{ $post->likes->contains('id', auth()->id()) ? 'bi-heart-fill text-danger' : 'bi-heart' }}"></i>
        ...
        <div class="comment-section mt-3">
            ...
            <ul>
                ...
                <li>
                    ...
                    <form method="POST" action="{{ route('posts.comments.destroy', $comment->id) }}"
                          style="display:inline; background:transparent; padding:0; margin:0; border:none;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="delete-comment-btn" aria-label="Elimina commento">
                            &times;
                        </button>
                    </form>
                    @endif
